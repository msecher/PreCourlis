"""
Contains the clas MascaretGeoFile
"""
...
from PreCourlis.lib.mascaret.mascaret_file import MascaretException
        ...
        with open(self.file_name, 'r') as filein:
            ...
            for line in filein:
                if line.startswith('#'):
                    ...
                    if self.has_ref:
                        _, reach_name, section_name, pk_str, _, _, _, _,\
            ...
            for _, reach in self.reaches.items():
                ...
                for sec in reach:
                    ...
                    if not ref and not layers:
                        ...
                        for i, (dist, x, y, z, topo_bath) in \
                                enumerate(zip(sec.distances, sec.x, sec.y,
                                              sec.z, sec.topo_bath)):
                            if self.nlayers == 0:
                                layers_str = ''
                            else:
